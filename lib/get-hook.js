...
function getIndexed() {
  return _.map(_.compact(execSync('(cd ' + gitParent + ' && git diff --cached --name-only --diff-filter=ACM)', { silent: true }).output.split('\n')), function(file) {
    var filePath = path.resolve(path.dirname(file));
    return path.format({
      dir: path.relative(filePath, gulpParent),
...
function streamFromIndexed(gulp, options) {
  ...
  return gulp.src(getIndexed(), options)
      .pipe(map(function (file, cb) {
        ...
        var hash = execSync('(cd ' + gitParent + ' && git ls-files -s ' + file.path + ')', {
          silent: true
        }).output.split(' ')[1];
        ...
        var contents = execSync('(cd ' + gitParent + ' && git cat-file blob ' + hash + ')', {
          silent: true
        }).output;
